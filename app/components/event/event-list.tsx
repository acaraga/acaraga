...
// 1. Update Interface biar support Object Location dari Live Server
interface LocationObject {
  city: string;
  name: string;
  // property lain abaikan dulu
}
...
interface Event {
  ...
  description: string;
  // Location bisa berupa String (lokal) ATAU Object (live)
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
      {displayedEvents.map((event) => {
        ...
        if (typeof event.location === "string") {
          ...
          locationName =
            event.location.city || event.location.name || "Indonesia";
        }
        ...
        return (
          ...
          >
            <div className="relative aspect-[4/3] w-full overflow-hidden bg-muted">
              <img
                src={
                  event.imageUrl || "https://placehold.co/600x400?text=No+Image"
                }
                alt={event.name}
                ...
                onError={(e) => {
                  e.currentTarget.src =
                    "https://placehold.co/600x400?text=Broken+Image";
                }}
              />
            </div>
            ...
            <CardContent className="pt-4 space-y-2 flex-1">
              <div className="flex justify-between items-start">
                <p className="text-[10px] font-medium text-blue-600 uppercase tracking-wider bg-blue-50 px-2 py-1 rounded-md">
                  {event.categorySlug}
                </p>
              </div>
              ...
              <h2 className="font-bold text-lg leading-tight line-clamp-2 min-h-[3.5rem]">
                {event.name}
              </h2>
